...
var _ = FDescribe("up", func() {
	...
	It("bbl's up a new bosh director and jumpbox", func() {
		...
		By("verifying that artifacts are created in state dir", func() {
			...
			_, err = os.Stat(filepath.Join(stateDir, "bosh", "cloudconfig", "cloud-config.yml"))
			Expect(err).NotTo(HaveOccurred())
			_, err = os.Stat(filepath.Join(stateDir, "bosh", "cloudconfig", "ops.yml"))
			Expect(err).NotTo(HaveOccurred())

			_, err = os.Stat(filepath.Join(stateDir, "bosh", "bosh-deployment", "aws-bosh-director-encrypt-disk-ops.yml"))
			Expect(err).NotTo(HaveOccurred())
			_, err = os.Stat(filepath.Join(stateDir, "bosh", "bosh-deployment", "aws-bosh-director-ephemeral-ip-ops.yml"))
			Expect(err).NotTo(HaveOccurred())
			_, err = os.Stat(filepath.Join(stateDir, "bosh", "bosh-deployment", "aws-external-ip-not-recommended.yml"))
			Expect(err).NotTo(HaveOccurred())
			_, err = os.Stat(filepath.Join(stateDir, "bosh", "bosh-deployment", "azure-external-ip-not-recommended.yml"))
			Expect(err).NotTo(HaveOccurred())
			_, err = os.Stat(filepath.Join(stateDir, "bosh", "bosh-deployment", "azure-ssh-static-ip.yml"))
			Expect(err).NotTo(HaveOccurred())
			_, err = os.Stat(filepath.Join(stateDir, "bosh", "bosh-deployment", "bosh.yml"))
			Expect(err).NotTo(HaveOccurred())
			_, err = os.Stat(filepath.Join(stateDir, "bosh", "bosh-deployment", "cpi.yml"))
			Expect(err).NotTo(HaveOccurred())
			_, err = os.Stat(filepath.Join(stateDir, "bosh", "bosh-deployment", "credhub.yml"))
			Expect(err).NotTo(HaveOccurred())
			_, err = os.Stat(filepath.Join(stateDir, "bosh", "bosh-deployment", "gcp-bosh-director-ephemeral-ip-ops.yml"))
			Expect(err).NotTo(HaveOccurred())
			_, err = os.Stat(filepath.Join(stateDir, "bosh", "bosh-deployment", "gcp-external-ip-not-recommended.yml"))
			Expect(err).NotTo(HaveOccurred())
			_, err = os.Stat(filepath.Join(stateDir, "bosh", "bosh-deployment", "iam-instance-profile.yml"))
			Expect(err).NotTo(HaveOccurred())
			_, err = os.Stat(filepath.Join(stateDir, "bosh", "bosh-deployment", "jumpbox-user.yml"))
			Expect(err).NotTo(HaveOccurred())
			_, err = os.Stat(filepath.Join(stateDir, "bosh", "bosh-deployment", "uaa.yml"))
			Expect(err).NotTo(HaveOccurred())
			_, err = os.Stat(filepath.Join(stateDir, "bosh", "bosh-deployment", "user-ops-file.yml"))
			Expect(err).NotTo(HaveOccurred())

			_, err = os.Stat(filepath.Join(stateDir, "bosh", "jumpbox-deployment", "cpi.yml"))
			Expect(err).NotTo(HaveOccurred())
			_, err = os.Stat(filepath.Join(stateDir, "bosh", "jumpbox-deployment", "jumpbox.yml"))
			Expect(err).NotTo(HaveOccurred())

			_, err = os.Stat(filepath.Join(stateDir, "bosh", "previous-user-ops-file.yml"))
			Expect(err).NotTo(HaveOccurred())
			...
			Expect(err).NotTo(HaveOccurred())
			_, err = os.Stat(filepath.Join(stateDir, "vars", "bosh-deployment-variables.json"))
			Expect(err).NotTo(HaveOccurred())
			_, err = os.Stat(filepath.Join(stateDir, "vars", "jumpbox-deployment-state.json"))
			Expect(err).NotTo(HaveOccurred())
			_, err = os.Stat(filepath.Join(stateDir, "vars", "jumpbox-deployment-variables.json"))
			Expect(err).NotTo(HaveOccurred())
